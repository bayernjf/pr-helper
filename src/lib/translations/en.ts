import type { TranslationDict } from '../i18n';

const en: TranslationDict = {
  // Brand
  'brand.name': 'PR',
  'brand.suffix': 'FLOW',

  // Nav
  'nav.label': 'Main navigation',
  'nav.overview': 'Flows',
  'nav.newFlow': '+ New Flow',

  // Theme
  'theme.light': 'Light',
  'theme.dark': 'Dark',
  'theme.toLight': 'Switch to light theme',
  'theme.toDark': 'Switch to dark theme',

  // Language
  'lang.label': 'Language',
  'lang.en': 'EN',
  'lang.zh': '中文',

  // Connect / Onboarding
  'connect.eyebrow': 'PR FLOW',
  'connect.eyebrow.github': 'GITHUB',
  'connect.hero.title': 'Track PR pipelines and merge gates in one place.',
  'connect.hero.sub': 'Connect GitHub → Pick repos → Create flows.',
  'connect.card.eyebrow': 'SECURE CONNECTION',
  'connect.card.title': 'Connect GitHub',
  'connect.card.intro': 'Authorize PR Helper to access selected repositories. You can adjust scope anytime.',
  'connect.card.button': 'Connect GitHub',
  'connect.card.arrow': '→',
  'connect.benefit1': 'Supports public, private, and organization repositories',
  'connect.benefit2': 'Grant access to all or selected repos; revoke anytime on GitHub',
  'connect.benefit3': 'GitHub access tokens are never stored in the browser',
  'connect.pat.summary': 'Use Personal Access Token (local dev only)',
  'connect.pat.label': 'GitHub Personal Access Token',
  'connect.pat.placeholder': 'github_pat_…',
  'connect.pat.meta': 'Token is stored only in the current browser session for local development.',
  'connect.pat.button': 'Connect with PAT',
  'connect.hint.local': 'Local Vite preview does not run the GitHub App auth API. Set VITE_AUTH_ORIGIN to your Vercel URL, or use a PAT below for local development.',
  'connect.error.generic': 'Connection failed',
  'connect.loading': 'Loading your workspace…',
  'connect.error.repos': 'Unable to load repositories',
  'connect.draft.saveError': 'Draft save failed',

  // Local Vite mode
  'localMode.title': 'Local Vite mode',
  'localMode.desc': 'The local development server does not run cloud APIs. Flows are stored only in this browser. Use vercel dev to test cloud sync, queues, and notifications.',
  'localMode.apiUnavailable': 'Local Vite is not running cloud APIs. Use vercel dev to test this feature.',

  // Topbar / Account
  'account.label': 'Account & Settings',
  'account.manageRepos': 'Manage repositories ↗',
  'account.push.on': 'Notifications on',
  'account.push.off': 'Enable notifications',
  'account.push.title': 'Browser notifications are enabled',
  'account.aiSettings': 'AI Settings',
  'account.disconnect': 'Disconnect GitHub',
  'account.deleteAccount': 'Delete Account',
  'account.privacy': 'Privacy Policy',
  'account.permissions': 'GitHub Permissions',
  'account.operationAudit': 'Operation audit',
  'account.teams': 'Team collaboration',

  // Teams
  'teams.eyebrow': 'TEAM COLLABORATION',
  'teams.title': 'Teams & shared flows',
  'teams.desc': 'Members can operate shared flows only within their assigned role.',
  'teams.create.label': 'Create team',
  'teams.create.placeholder': 'For example: Platform engineering',
  'teams.create.button': 'Create team',
  'teams.empty': 'No teams yet. Create one to invite members and share flows.',
  'teams.members.title': 'Members',
  'teams.members.empty': 'This team has no members.',
  'teams.members.login': 'GitHub login',
  'teams.members.role': 'Role',
  'teams.members.add': 'Add or update member',
  'teams.members.remove': 'Remove',
  'teams.share.title': 'Share flow',
  'teams.share.empty': 'There are no personal flows to share.',
  'teams.share.select': 'Choose a personal flow',
  'teams.share.button': 'Share with team',
  'teams.ownerOnly': 'Only Owners can manage members and share flows.',
  'teams.role.owner': 'Owner',
  'teams.role.editor': 'Editor',
  'teams.role.operator': 'Operator',
  'teams.role.viewer': 'Viewer',
  'teams.sharedBadge': 'Team · {team} · {role}',
  'teams.close': 'Close',

  // Operation audit
  'audit.eyebrow': 'OPERATION AUDIT',
  'audit.title': 'Operation audit',
  'audit.desc': 'Records key workflow operations initiated by this account and their result. Access tokens, PR bodies, and AI drafts are never included.',
  'audit.loading': 'Loading operation records…',
  'audit.empty': 'No operation records to show yet.',
  'audit.error': 'Unable to load the operation audit.',
  'audit.export': 'Export CSV',
  'audit.close': 'Close',
  'audit.unknown': 'No repository',
  'audit.outcome.success': 'Succeeded',
  'audit.outcome.failure': 'Failed',
  'audit.action.workflow-created': 'Created workflow',
  'audit.action.workflow-updated': 'Updated workflow',
  'audit.action.workflow-deleted': 'Deleted workflow',
  'audit.action.pull-created': 'Created PR',
  'audit.action.pull-merged': 'Merged PR',
  'audit.action.actions-rerun': 'Reran Actions',
  'audit.action.deployment-rerun': 'Reran deployment',
  'audit.action.deployment-rollback': 'Triggered deployment rollback',

  // Overview
  'overview.eyebrow.queue': 'NEEDS YOUR ATTENTION',
  'overview.eyebrow.workspace': 'WORKSPACE',
  'overview.hero.title': 'We only interrupt you when a decision is needed.',
  'overview.hero.sub': 'Cross-repo PR orchestration, gating, and to-dos — all in one place.',
  'overview.hero.createFlow': 'Create Flow',
  'overview.savedFlows': 'SAVED FLOWS',
  'overview.flowsCount': '{count} saved flows',
  'overview.noFlows': 'No flows yet',
  'overview.empty.title': 'Start with a repository',
  'overview.empty.desc': 'Pick real branches and configure release pipelines like feature → dev → main.',
  'overview.empty.button': 'Create your first flow',
  'overview.queue.title': 'Needs your attention · {count}',
  'overview.queue.refresh': 'Refresh queue',
  'overview.queue.refreshing': 'Refreshing…',
  'overview.flowCard.view': 'View →',
  'overview.flowCard.steps': '{count} steps · Not started',
  'overview.queue.step': 'Step {index}',
  'overview.queue.error.title': 'To-do queue is temporarily unavailable',
  'overview.board.eyebrow': 'PROJECT WORKSPACE',
  'overview.board.title': 'Project workflow board',
  'overview.board.sub': 'See every project’s PR flow, gates, and current position in one place.',
  'overview.board.addProject': '+ Add project',
  'overview.board.summary': 'Project status filters',
  'overview.board.search': 'Search',
  'overview.board.searchCount': 'Search results',
  'overview.board.searchPlaceholder': 'Search by workflow or repository',
  'overview.board.searchEmpty': 'No matching workflows',
  'overview.board.searchClear': 'Clear search',
  'overview.board.attention': 'Needs attention',
  'overview.board.running': 'Running',
  'overview.board.active': 'All projects',
  'overview.board.failed': 'Failed',
  'automation.state.queued': 'Queued, waiting to run',
  'automation.state.running': 'Running',
  'automation.state.blocked': 'Paused, needs attention',
  'automation.state.failed': 'Execution failed',
  'automation.kind.create-pr': 'Auto-create PR',
  'automation.kind.merge-pr': 'Auto-merge PR',
  'automation.attempts': '{count} attempt(s)',
  'automation.blockedBadge': 'Automation blocked',
  'automation.blockedTooltip': 'Automation for this step has stopped. Open the step detail for the reason.',
  'automation.reasonUnknown': 'No reason recorded',
  'automation.updated': 'Updated {time}',
  'automation.detail.eyebrow': 'Automation queue',
  'automation.detail.empty': 'No unfinished automation action for this step.',
  'overview.board.automationBlocked': 'Automation blocked',
  'overview.board.filterEmpty': 'No projects match this filter',
  'overview.board.archived': 'Archived',
  'overview.board.archivedEmpty': 'No archived projects yet',
  'overview.board.archivedBadge': 'Archived',
  'overview.board.archive': 'Archive',
  'overview.board.restore': 'Restore',
  'overview.board.showAll': 'Show all projects',
  'overview.board.sortLabel': 'Sort',
  'overview.board.sortCustom': 'Custom',
  'overview.board.sortName': 'Flow name',
  'overview.board.sortCreated': 'Created',
  'overview.board.scrollControls': 'Board scroll controls',
  'overview.board.scrollTop': 'Scroll to top',
  'overview.board.scrollBottom': 'Scroll to bottom',
  'overview.board.collapse': 'Collapse lane details',
  'overview.board.expand': 'Expand lane details',
  'overview.board.waitingSync': 'Waiting for status sync',
  'overview.board.edit': 'Edit',
  'overview.board.dragProject': 'Drag to reorder: {{name}}',
  'overview.board.moveUp': 'Move project up: {{name}}',
  'overview.board.moveDown': 'Move project down: {{name}}',
  'overview.board.order': 'Order',
  'overview.board.orderFor': 'Flow order: {{name}}',
  'overview.board.sortAllOnly': 'Switch to All projects to change the order',
  'overview.board.mergeTarget': 'Merges into {target}',
  'overview.run.waiting-sync': 'Waiting for status sync',
  'overview.run.waiting-for-changes': 'Waiting for new commits',
  'overview.run.ready-to-create': 'Ready to create PR',
  'overview.run.checks-running': 'Checks running',
  'overview.run.checks-passed': 'Checks passed',
  'overview.run.checks-failed': 'Actions failed',
  'overview.run.merged-pending': 'Merged · waiting for post-merge checks',
  'overview.run.prStatus': 'PR #{number} · {status}',
  'overview.run.current': 'Current: step {step}/{total} · {status}',
  'overview.run.updated': 'Synced {time}',
  'overview.run.checkCount': 'Checks: {passed}/{total} passed',
  'overview.run.history': 'Recent activity',
  'overview.run.createPr': 'Create PR',
  'overview.deployment.title': 'PUBLIC DEPLOYMENTS',
  'overview.deployment.history': 'DEPLOYMENT RUN HISTORY',
  'overview.deployment.preview': 'Preview',
  'overview.deployment.production': 'Production',
  'overview.deployment.pending': 'Deploying',
  'overview.deployment.success': 'Deployed',
  'overview.deployment.failure': 'Deployment failed',
  'overview.deployment.openSite': 'Open site',
  'overview.deployment.openLogs': 'Open logs',
  'overview.deployment.openFailedJob': 'Open failed step',
  'overview.deployment.missing': 'Workflow not found',
  'overview.deployment.retry': 'Redeploy',
  'overview.deployment.retrying': 'Redeploying…',
  'overview.deployment.retryStarted': '{provider} deployment was triggered again.',
  'overview.deployment.health': 'Health check',
  'overview.deployment.healthPassed': 'passed',
  'overview.deployment.healthFailed': 'failed',
  'overview.deployment.rollback': 'Roll back to this version',
  'overview.configWarning.title': 'CONFIGURATION & RUNTIME NOTES',
  'overview.configWarning.count': '{count} configuration notes',
  'overview.configWarning.no-deployments': 'No public deployment gate is configured, so this flow will not wait for a public deployment.',
  'overview.configWarning.actions-unavailable': 'GitHub Actions cannot be read. Check the GitHub App Actions: Read permission and repository access.',
  'overview.configWarning.workflow-not-found': 'Workflow “{value}” was not found for target {target}.',
  'overview.configWarning.environment-missing': 'No GitHub Environment is set for target {target}.',
  'overview.configWarning.environment-not-found': 'GitHub Environment “{value}” was not found in this repository.',
  'overview.configWarning.rollback-workflow-not-found': 'Rollback workflow “{value}” was not found.',
  'overview.configWarning.deployment-not-seen': '{provider}: no run for “{value}” was found more than 10 minutes after merge.',
  'overview.configWarning.deployment-stuck': '{provider}: “{value}” has been running for over 20 minutes. Open Actions to inspect the wait reason.',
  'overview.board.close': 'Close',
  'overview.board.stepDetail': 'STEP DETAIL',
  'overview.board.noPull': 'No PR exists for this step yet',
  'overview.board.viewDetail': 'View full flow',
  'toast.order.saved': 'Project order saved',
  'toast.order.local': 'Project order was saved locally; cloud sync failed: {{error}}',

  // Storage sync
  'sync.warning.title': 'Cloud flow sync failed',
  'sync.warning.desc': 'Current flows are only saved in this browser.',
  'sync.warning.detail': 'View details',
  'sync.loading.title': 'Syncing cloud status',
  'sync.loading.desc': 'The board is ready to use while saved flows and queue snapshots load in the background.',
  'sync.prompt.title': 'Found {count} flows stored only on this device',
  'sync.prompt.desc': 'Syncing to your GitHub account @{login} makes them available on other devices.',
  'sync.prompt.button': 'Sync to account',
  'sync.success': 'Local flows synced to your GitHub account.',
  'sync.fail': 'Flow sync failed, local data preserved: {error}',
  'sync.fail.short': 'Flow sync failed',

  // Editor
  'editor.eyebrow': 'FLOW EDITOR',
  'editor.back.detail': '← Back',
  'editor.back.overview': '← Back',
  'editor.title.edit': 'Edit Flow',
  'editor.title.new': 'New Flow',
  'editor.subtitle': 'Configuring a flow does not create any GitHub PRs.',
  'editor.label.name': 'Flow name',
  'editor.placeholder.name': 'Defaults to the repository name if left blank',
  'editor.label.repo': 'Repository',
  'editor.repo.placeholder': 'Select a GitHub repository',
  'editor.repo.private': ' · private',
  'editor.repo.used': ' · workflow configured',
  'editor.branch.loading': 'Loading branches…',
  'editor.branch.hint': 'Select a repository to see actual branches.',
  'editor.label.source': 'Source',
  'editor.sourceRuleHint': 'Enter an exact branch or a rule such as feature/* or fix/*.',
  'editor.label.target': 'Target',
  'editor.compare': 'View Compare on GitHub ↗',
  'editor.addStep': 'Add step',
  'editor.addRoute': 'Add merge route',
  'editor.saveFlow': 'Save flow',
  'editor.cancelStep': 'Cancel',
  'editor.repoSwitch.eyebrow': 'SWITCH REPOSITORY',
  'editor.repoSwitch.title': 'Switch to {repository}?',
  'editor.repoSwitch.desc': 'A flow maps to a single repository. Switching to {repository} starts a new flow; the existing flow “{name}” stays exactly as it is and is not migrated.',
  'editor.repoSwitch.cancel': 'Cancel',
  'editor.repoSwitch.confirm': 'Start a new flow',
  'editor.independent.label': 'Independent merge route',
  'editor.independent.desc': 'This route can create and merge its PR without waiting for an earlier route.',
  'editor.routeMode.editing': 'Editing route {route}',
  'editor.routeMode.label': 'Route trigger condition',
  'editor.routeMode.sequential': 'Wait for the previous step to merge',
  'editor.routeMode.independent': 'Independent route, does not wait for the previous step',
  'editor.routeMode.waitFor': 'Wait for selected routes',
  'editor.routeMode.waitForEmpty': 'Select at least one route to wait for.',
  'editor.routeMode.update': 'Update route',
  'editor.routeMode.cancel': 'Cancel',
  'editor.routeMode.updated': 'Updated the trigger condition for {route}.',
  'editor.dependencies.label': 'Release convergence gate (optional)',
  'editor.dependencies.desc': 'This PR can be created only after every selected route merges and its post-merge Actions pass.',
  'editor.error.branches': 'Unable to load branches',
  'editor.error.sameBranch': 'Source and Target cannot be the same branch.',
  'editor.error.duplicateRoute': 'This merge route already exists.',
  'editor.error.repoUsed': 'This repository already has a workflow. Choose another repository.',
  'editor.toast.saved': 'Flow "{name}" saved.',
  'editor.toast.stepSaved': 'Saved step {step}: {source} → {target}',
  'editor.toast.routeSaved': 'Saved merge route: {source} → {target}',
  'editor.deployments.label': 'Public deployment gates',
  'editor.deployments.desc': 'Match GitHub Actions by target branch; every configured deployment must succeed before the next step unlocks.',
  'editor.deployments.empty': 'No public deployment gate is configured for this project.',
  'editor.deployments.target': 'Target branch',
  'editor.deployments.provider': 'Deployment platform',
  'editor.deployments.workflow': 'GitHub Actions workflow name',
  'editor.deployments.workflowHint': 'Choose a workflow from this repository or enter a name manually.',
  'editor.deployments.workflowUnavailable': 'No selectable workflows were loaded; enter a workflow name manually.',
  'editor.deployments.environment': 'Deployment environment',
  'editor.deployments.githubEnvironment': 'GitHub Environment (optional)',
  'editor.deployments.githubEnvironmentHint': 'Pick one of the repository Environments; leave it empty to derive the conventional name.',
  'editor.deployments.githubEnvironmentNone': 'This repository has no Environment, so leave it empty.',
  'editor.deployments.githubEnvironmentBlank': 'leave empty',
  'editor.deployments.githubEnvironmentUnavailable': 'No repository Environments were loaded; leaving it empty is fine, or enter a name manually.',
  'editor.deployments.preview': 'Preview',
  'editor.deployments.production': 'Production',
  'editor.deployments.add': 'Add deployment gate',
  'editor.deployments.suggestLabel': 'Deploy workflows in this repository',
  'editor.deployments.suggestHint': 'Filled in from what the repository has. Check the branch, platform and Environment before saving.',
  'editor.deployments.suggestApply': 'Use this',
  'editor.deployments.sync': 'Sync from repository',
  'editor.deployments.syncHint': 'Keeps the gates whose workflow the repository has and removes the rest.',
  'editor.deployments.syncUnavailable': 'The repository workflow list could not be read, so there is nothing to sync against.',
  'editor.deployments.syncUnchanged': 'The deployment gates already match the repository.',
  'editor.deployments.synced': 'Synced from the repository: {kept} kept, {added} added, {removed} removed.',
  'editor.deployments.cancel': 'Cancel',
  'editor.deployments.submit': 'Save gate',
  'editor.deployments.advanced': 'Advanced options',
  'editor.deployments.remove': 'Remove',
  'editor.deployments.edit': 'Edit',
  'editor.deployments.update': 'Update gate',
  'editor.deployments.updated': 'Deployment gate updated.',
  'editor.deployments.editing': 'Editing {workflow} · {target}',
  'editor.deployments.saved': 'Deployment gate saved.',
  'editor.deployments.undo': 'Undo',
  'editor.deployments.removed': 'Deployment gate removed.',
  'editor.deployments.workflowRequired': 'Enter the GitHub Actions workflow name.',
  'editor.deployments.duplicate': 'Only one deployment platform may be configured for a target branch.',
  'editor.deployments.healthPath': 'Health check path (optional)',
  'editor.deployments.healthPathHint': 'Runs once against the public URL returned by this deployment, for example /health. A failed probe blocks the next stage and is not retried automatically — a new commit is needed to re-evaluate it.',
  'editor.deployments.healthPathInvalid': 'The health check path must start with /.',
  'editor.deployments.rollbackWorkflow': 'Rollback Actions workflow (optional)',
  'editor.deployments.rollbackWorkflowHint': 'Shown only when configured. Its workflow_dispatch must accept target_run_id, deployment_url, environment, and provider.',
  'editor.deployments.rollbackSummary': 'Rollback: {workflow}',
  'editor.deployments.warningCount': '{count} item(s) need attention before saving',
  'editor.deployments.configOk': 'The deployment configuration passed basic validation.',
  'editor.deployments.warning.no-deployments': 'No deployment gate is configured; the flow will not wait for Vercel or Cloudflare after merge.',
  'editor.deployments.warning.actions-unavailable': 'Unable to load GitHub Actions. Check the Actions: Read permission.',
  'editor.deployments.warning.workflow-not-found': 'Deployment workflow “{value}” was not found in this repository.',
  'editor.deployments.warning.environment-missing': 'No GitHub Environment is set, so PR Helper must use an inferred name.',
  'editor.deployments.warning.environment-not-found': 'GitHub Environment “{value}” was not found in this repository.',
  'editor.deployments.warning.health-path-invalid': 'Health check path “{value}” must start with /.',
  'editor.deployments.warning.rollback-workflow-not-found': 'Rollback workflow "{value}" was not found in this repository.',

  // Recovery policy editor
  'editor.recoveryPolicy.label': 'RECOVERY POLICY',
  'editor.recoveryPolicy.desc': 'Configure retry limits and cooldown for failed Actions. Manual intervention is needed when limits are exceeded.',
  'editor.recoveryPolicy.maxRetries': 'Max retries',
  'editor.recoveryPolicy.cooldownSeconds': 'Cooldown (seconds)',
  'editor.recoveryPolicy.save': 'Save recovery policy',
  'editor.recoveryPolicy.saved': 'Recovery policy saved.',

  // Deployment rollback
  'rollback.eyebrow': 'CONFIRM DEPLOYMENT ROLLBACK',
  'rollback.title': 'Roll back this deployment?',
  'rollback.desc': 'GitHub Actions will roll back {provider} {environment}; PR Helper will not change the platform version directly.',
  'rollback.target': 'Rollback target',
  'rollback.workflow': 'Workflow: {workflow}',
  'rollback.warning': 'Rollback is never automatic. It starts only after confirmation and is recorded in the audit history.',
  'rollback.cancel': 'Cancel',
  'rollback.confirm': 'Confirm and start rollback',
  'rollback.starting': 'Starting…',
  'rollback.started': 'Rollback workflow started: {workflow}',
  'rollback.failed': 'Unable to start deployment rollback.',
  'rollback.unavailable': 'No usable rollback workflow is configured for this deployment.',

  // Draft panel
  'draft.eyebrow': 'FLOW DRAFT',
  'draft.empty.title': 'No steps saved yet',
  'draft.empty.desc': 'Once you save the first step, the flow will appear here.',
  'draft.remove': 'Remove',
  'draft.editRoute': 'Edit',
  'draft.saving': 'Saving…',
  'draft.independent': 'Independent',
  'draft.autoCreate': 'Auto-create PR',
  'draft.autoCreateDesc': 'This step may create a PR through automation (PoC stores policy only).',
  'draft.autoCreatePrerequisites': 'Configure the server credential, enable auto-generate and auto-confirm, and keep at least one generation rule first.',
  'draft.autoCreateEnabled': 'Auto-create policy enabled for this step.',
  'draft.autoCreateDisabled': 'Auto-create policy disabled for this step.',
  'draft.autoCreateThreshold': 'Trigger after',
  'draft.autoCreateThresholdTooltip': 'Number of new commits required to trigger automatic PR creation',
  'draft.autoCreateThresholdInvalid': 'The commit threshold must be a positive integer.',
  'draft.autoCreateThresholdSaved': 'Auto-create trigger threshold updated to {count} commits.',
  'draft.autoMerge': 'Auto-merge PR',
  'detail.autoMergeDesc': "Merges this step's PR once every gate is green, whether the PR came from automation or a person. Merge commit only.",
  'detail.autoMergeReadonly': 'You do not have permission to merge PRs in this workflow.',
  'detail.autoMergeLegacyPolicy': 'This step still uses the legacy browser-side auto-create policy. Turn auto-create off and on again before enabling auto-merge.',
  'draft.autoMergeEnabled': 'Auto-merge policy enabled for this step.',
  'draft.autoMergeDisabled': 'Auto-merge policy disabled for this step.',
  'detail.autoCreateDesc': 'Creates a PR in the background when conditions are met.',
  'detail.autoCreateReadonly': 'You do not have permission to edit this workflow.',
  'detail.ruleOutdated.content': 'The default generation rule "{name}" was updated; this step still generates from the older version.',
  'detail.ruleOutdated.rule': 'The default generation rule is now "{name}"; this step is still pinned to "{stored}".',
  'detail.ruleOutdated.sync': 'Sync',
  'detail.ruleOutdated.synced': 'This step now uses the generation rule "{name}".',
  'automation.executeCreate': 'Run auto-create PR',
  'automation.queueFailed': 'Unable to queue the automation action.',
  'automation.executeFailed': 'Automatic PR creation failed.',
  'automation.executeSucceeded': 'PR #{number} was created automatically.',
  'automation.confirm.eyebrow': 'AUTOMATION POC',
  'automation.confirm.title': 'Run automatic PR creation?',
  'automation.confirm.desc': 'This uses rule “{rule}” to generate content and creates a real GitHub PR for {source} → {target}.',
  'automation.confirm.cancel': 'Cancel',
  'automation.confirm.confirm': 'Run now',
  'automation.tick.eyebrow': 'TAKES EFFECT NOW',
  'automation.tick.title': 'This runs as soon as you save',
  'automation.tick.createDesc': '{source} → {target} already has {count} commit(s) waiting, so saving creates a pull request for it right away.',
  'automation.tick.mergeDesc': 'PR #{number} ({source} → {target}) is open, so saving tries to merge it into {target} right away. It pauses on its own if a gate has not passed.',
  'automation.tick.cancel': 'Do not save',
  'automation.tick.confirm': 'Confirm and save',
  'draft.waitFor': 'Wait for {count} routes',
  'draft.drag': 'Drag to reorder: {name}',
  'draft.moveUp': 'Move step up: {name}',
  'draft.moveDown': 'Move step down: {name}',
  'draft.viewDetail': 'View flow details',

  'workflowDelete.action': 'Delete flow',
  'workflowArchive.eyebrow': 'Stops reconciliation',
  'workflowArchive.title': 'Archive this flow?',
  'workflowArchive.desc': 'The “{name}” flow moves off the board into the archived view, and PR Helper stops reconciling it against GitHub.',
  'workflowArchive.warning': 'Automation actions still waiting to run are cancelled. History, steps and versions are kept, and you can restore the flow at any time.',
  'workflowArchive.cancel': 'Cancel',
  'workflowArchive.confirm': 'Archive flow',
  'workflowArchive.archived': '“{name}” archived',
  'workflowArchive.restored': '“{name}” restored',
  'workflowDelete.eyebrow': 'Danger zone',
  'workflowDelete.title': 'Delete this flow?',
  'workflowDelete.desc': 'This will delete the “{name}” flow and its step configuration.',
  'workflowDelete.warning': 'GitHub pull requests, branches, and deployments will not be deleted. This cannot be undone.',
  'workflowDelete.cancel': 'Cancel',
  'workflowDelete.confirm': 'Delete flow',
  'workflowDelete.success': 'Flow deleted',

  // Detail
  'detail.eyebrow': 'FLOW DETAIL',
  'detail.timeline.eyebrow': 'EXECUTION TIMELINE',
  'detail.progress.eyebrow': 'FLOW PROGRESS',
  'detail.progress.position': 'Step {step} of {total} · {completed} done',
  'detail.progress.allDone': 'All {total} steps done · waiting for new commits',
  'detail.progress.waitingSync': 'Waiting for status sync',
  'detail.progress.syncedAt': 'Server projection · {time}',
  'detail.progress.step': 'Step {step}',
  'detail.progress.node.succeeded': 'Done',
  'detail.progress.node.failed': 'Failed',
  'detail.progress.node.blocked': 'Paused',
  'detail.progress.node.waiting-gates': 'Waiting on gates',
  'detail.progress.node.running': 'In progress',
  'detail.progress.node.ready': 'Actionable',
  'detail.progress.node.locked': 'Waiting upstream',
  'detail.progress.node.idle': 'Waiting for sync',
  'detail.nextAction.eyebrow': 'NEXT ACTION',
  'detail.refresh': 'Refresh GitHub status',
  'detail.refresh.loading': 'Loading…',
  'detail.edit': 'Edit flow',
  'detail.desc.withStatuses': 'Status comes directly from GitHub. When gates pass, you can create or merge PRs here.',
  'detail.desc.noStatuses': 'Click "Refresh GitHub status" to load PRs, Actions, and approvals for each step.',
  'detail.timeline.placeholder': 'GitHub status not yet loaded.',
  'detail.dynamicRoute': 'Dynamic branch rules are shown on the project board for each matched branch.',
  'detail.timeline.refreshError': 'Unknown error',

  // Status labels
  'status.waitingPr': 'Waiting for PR creation',
  'status.noPr': 'No PR exists on GitHub yet.',
  'status.createPr': 'Create PR',
  'status.createPrLink': 'Create PR on GitHub ↗',
  'status.locked': 'Waiting for the previous step to merge and post-merge checks to pass.',
  'status.locked.noChecks': 'Waiting for the previous step to merge.',
  'status.locked.deployment': 'Waiting for the “{workflow}” deployment, but no such Actions workflow exists in the repository — fix the deployment name in the workflow settings.',
  'status.fetchFailed': 'Fetch failed',
  'status.sourceBranchMissing': 'The Source branch does not exist or was deleted',
  'status.sourceBranchDeletedHint': 'Source branch deleted; showing the historical PR status',
  'status.targetBranchMissing': 'The Target branch does not exist or was deleted',
  'status.actions.passed': 'passed',
  'status.actions.failed': 'failed',
  'status.actions.running': 'running',
  'status.checks.summary': '{passed}/{total} Checks {state}',
  'status.checks.completed': 'completed',
  'status.checks.failed': 'failed',
  'status.checks.running': 'running',
  'status.checks.details': 'Check sources and details',
  'status.actions.runs.summary': '{passed}/{total} Actions {state}',
  'status.approvals': '{approvals}/{required} approvals',
  'status.merge.conflict': 'Merge conflict',
  'status.merge.behind': 'Branch needs update',
  'status.merge.blocked': 'GitHub gates not satisfied',
  'state.postMerge.passed': 'Post-merge checks passed',
  'state.postMerge.failed': 'Post-merge checks failed',
  'state.postMerge.running': 'Post-merge checks running',
  'state.merged': 'Merged',
  'state.closed': 'Closed',
  'state.actionsFailed': 'Actions failed',
  'state.waitingActions': 'Waiting for Actions',
  'state.waitingApprovals': 'Waiting for approvals',
  'state.mergeChecking': 'GitHub is checking whether this PR can merge',
  'state.mergeBlocked': 'Merge blocked',
  'state.waitingMerge': 'Waiting for merge',
  'status.newCommits': '{count} new commits',
  'status.newCommits.canCreate': 'A new PR can be created.',
  'status.newCommits.waitingChanges': 'Waiting for new commits on the Source branch.',
  'status.newCommits.waiting': 'Waiting for the previous step to merge and Actions to pass.',
  'status.createPr.button': 'Create new PR',
  'status.openPr': 'Open GitHub PR #{number} ↗',

  // Merge
  'merge.eyebrow': 'MERGE PULL REQUEST',
  'merge.merging': 'Merging…',
  'merge.button': 'Merge PR',
  'merge.selectMethod': 'Select merge method',
  'merge.commit.title': '✓  Merge commit (merge)',
  'merge.commit.desc': 'Keeps all commits from the branch and creates a merge commit.',
  'merge.squash.title': 'Squash and merge',
  'merge.squash.desc': 'Must be done on GitHub',
  'merge.rebase.title': 'Rebase and merge',
  'merge.error.generic': 'Merge failed',
  'merge.error.permission': 'The GitHub App lacks permission to merge this PR. Ensure Contents and Pull requests are both set to Read & write. After updating permissions, re-approve or reinstall PR Helper on GitHub, then retry.',
  'merge.dialog.title': 'Merge PR #{number}',
  'merge.dialog.desc': 'A merge commit will be created. GitHub will re-check permissions, branch protection, and latest commits.',
  'merge.dialog.cancel': 'Cancel',
  'merge.dialog.confirm': 'Confirm merge',
  'merge.error.incomplete': 'GitHub did not complete the merge.',

  // Codex repair
  'repair.eyebrow': 'CODEX REPAIR PACKAGE',
  'repair.collecting': 'Collecting repair context…',
  'repair.collecting.desc': 'Will include the PR, failed checks, Actions job links, and a diff summary. No code will be sent; no commits created.',
  'repair.error.title': 'Unable to generate Codex repair task',
  'repair.ready.title': 'Repair task ready',
  'repair.ready.desc': 'Copy and paste it into the Codex task for this repository to start the fix.',
  'repair.openActions': 'Open GitHub Actions ↗',
  'repair.copy': 'Copy to Codex',
  'repair.close': 'Close',
  'repair.error.generic': 'Unknown error',
  'repair.error.generate': 'Unable to generate repair task',
  'repair.toast.copied': 'Codex repair task copied.',
  'repair.toast.copyFailed': 'Copy failed — please copy the repair task manually.',
  'repair.codex': 'Send to Codex',
  'recovery.sync': 'Sync again',
  'recovery.syncing': 'Syncing…',
  'recovery.retryActions': 'Rerun Actions',
  'recovery.retrying': 'Rerunning…',
  'recovery.retryStarted': 'Reran {count} failed Actions.',
  'recovery.retryUnavailable': 'No failed Actions can be rerun.',
  'recovery.retryFailed': 'Unable to rerun Actions.',
  'recovery.retries': 'Retried {count}/{max}',
  'recovery.cooldown': 'Cooldown: {seconds}s remaining',
  'recovery.exhausted': 'Max retries reached ({count}), manual intervention needed',
  'recovery.escalation': '⚠️ Manual intervention needed',

  // Cloud sync
  'cloudSync.enable': '☁ Cloud Sync',
  'cloudSync.eyebrow': 'ENCRYPTED CLOUD SYNC',
  'cloudSync.title': 'Cloud Sync',
  'cloudSync.status.disabled': 'Disabled',
  'cloudSync.status.unlocked': 'Unlocked',
  'cloudSync.status.syncing': 'Syncing…',
  'cloudSync.status.error': 'Error',
  'cloudSync.unlock.desc': 'Enter a passphrase to encrypt your data before uploading. The server never sees plaintext.',
  'cloudSync.passphrase': 'Passphrase',
  'cloudSync.passphraseRequired': 'Please enter a passphrase.',
  'cloudSync.unlock': 'Unlock',
  'cloudSync.lock': 'Lock',
  'cloudSync.close': 'Close',
  'cloudSync.push': 'Upload',
  'cloudSync.pull': 'Download',
  'cloudSync.lastSynced': 'Last synced',
  'cloudSync.pushSuccess': 'Data uploaded to cloud.',
  'cloudSync.pushFailed': 'Upload failed.',
  'cloudSync.pullSuccess': 'Data restored from cloud.',
  'cloudSync.pullEmpty': 'No cloud data found.',
  'cloudSync.pullFailed': 'Download failed.',
  'cloudSync.deviceHint': 'This device uses its own identifier. On a new device, unlock first and then download the encrypted cloud data.',
  'cloudSync.rotate': 'Rotate passphrase',
  'cloudSync.rotateDesc': 'Re-encrypt current local data with a new passphrase and upload it. A conflict never overwrites cloud data.',
  'cloudSync.conflict': 'Cloud data was updated by another device. Download the latest ciphertext before deciding whether to upload local data again.',

  // AI settings
  'ai.eyebrow': 'AI MODEL SETTINGS',
  'ai.title': 'Configure AI model',
  'ai.label.baseUrl': 'API Base URL',
  'ai.placeholder.baseUrl': 'https://api.openai.com/v1',
  'ai.label.model': 'Model',
  'ai.placeholder.model': 'gpt-4.1-mini',
  'ai.label.apiKey': 'API Key',
  'ai.toggle.label': 'Auto-generate title and description on PR creation',
  'ai.toggle.desc': 'Only runs when model config is complete. You can still edit manually in the dialog.',
  'ai.toggle.autoConfirm.label': 'Automatically confirm PR creation after AI generation',
  'ai.toggle.autoConfirm.desc': 'Only applies to automatic generation. A successful generation immediately creates the PR on GitHub.',
  'ai.test.placeholder': 'Test the connection before saving.',
  'ai.test': 'Test connection',
  'ai.cancel': 'Cancel',
  'ai.save': 'Save settings',
  'ai.test.error.empty': 'Please fill in API Base URL and API Key first.',
  'ai.test.loading': 'Testing connection…',
  'ai.test.success': 'Connection successful — safe to save.',
  'ai.test.error.chars': 'Invalid API Key format — check for spaces, quotes, or non-ASCII characters.',
  'ai.test.error.generic': 'Unable to connect to the model service. Check the URL, Key, and network.',
  'ai.toast.saved': 'AI model settings saved for this session.',
  'ai.automation.title': 'Automation AI credential (PoC)',
  'ai.automation.desc': 'Stored encrypted on the server for future unattended workflows; it does not replace this session setting. The connection is tested before saving.',
  'ai.automation.loading': 'Loading server credential status…',
  'ai.automation.configured': 'Server credential configured',
  'ai.automation.notConfigured': 'No server credential configured',
  'ai.automation.save': 'Save model configuration',
  'ai.automation.update': 'Update model configuration',
  'ai.automation.saveTooltip': 'Saved AI credentials are used for automatic PR creation',
  'ai.automation.delete': 'Delete server credential',
  'ai.automation.error': 'Automation credential operation failed',
  'ai.automation.test': 'Test saved configuration',
  'ai.automation.testPlaceholder': 'Test the AI credential saved for automation.',
  'ai.automation.testLoading': 'Testing saved configuration…',
  'ai.automation.testSuccess': 'Server AI configuration connected',
  'ai.automation.testError': 'Saved AI configuration test failed',
  'ai.key.show': 'Show',
  'ai.key.hide': 'Hide',
  'ai.key.showLabel': 'Show API Key',
  'ai.key.hideLabel': 'Hide API Key',

  // Create PR
  'createPr.eyebrow': 'CREATE PULL REQUEST',
  'createPr.label.title': 'PR title',
  'createPr.label.body': 'PR description (optional)',
  'createPr.placeholder.body': 'Use AI to generate',
  'createPr.meta': 'PR is created on GitHub only after confirmation. No auto-merge.',
  'createPr.aiGenerate': 'AI Generate',
  'createPr.aiSettings': 'AI Settings',
  'createPr.cancel': 'Cancel',
  'createPr.confirm': 'Create PR',
  'createPr.aiLoading': 'AI generating title and description…',
  'createPr.creating': 'Creating Pull Request…',
  'createPr.generating': 'Generating…',
  'createPr.creatingShort': 'Creating…',
  'createPr.aiError': 'AI generation failed',
  'createPr.createError': 'Creation failed',
  'createPr.overwrite.title': 'Regenerate PR content?',
  'createPr.overwrite.desc': 'AI generation will overwrite the current PR title and description. This only affects the draft in this dialog — GitHub content is not modified.',
  'createPr.overwrite.cancel': 'Cancel',
  'createPr.overwrite.confirm': 'Continue',

  // Generation rules
  'rules.eyebrow': 'PR GENERATION RULES',
  'rules.title': 'Generation rules',
  'rules.aria.list': 'Generation rule list',
  'rules.badge.default': 'Default',
  'rules.empty': 'No generation rules yet.',
  'rules.addText': '+ Add text',
  'rules.import': 'Import .md',
  'rules.label.name': 'Rule name',
  'rules.placeholder.name': 'e.g., Standard PR',
  'rules.label.content': 'Markdown content',
  'rules.placeholder.content': '# Title rules\nUse concise English.',
  'rules.cancel': 'Cancel',
  'rules.save': 'Save',
  'rules.setDefault': 'Set as default',
  'rules.use': 'Use this rule',
  'rules.confirm.discard': 'You have unsaved changes. Discard?',
  'rules.error.empty': 'Rule content cannot be empty',
  'rules.error.import': 'Unable to read Markdown file',
  'rules.error.save': 'Unable to save rule',
  'rules.error.default': 'Unable to set default rule',
  'rules.error.use': 'Save all changes before using this rule',

  // Notification permission dialog
  'notify.title': 'Allow browser notifications',
  'notify.desc': 'Click the site info icon left of the address bar, go to "Site settings", and set "Notifications" to "Allow". Return to this page and click "Enable notifications" again.',
  'notify.confirm': 'Got it',

  // Disconnect dialog
  'disconnect.title': 'Disconnect GitHub?',
  'disconnect.desc': 'This clears the GitHub login session in this browser. Saved local flows are not deleted.',
  'disconnect.cancel': 'Cancel',
  'disconnect.confirm': 'Confirm disconnect',

  // Toast
  'toast.saved.local': 'Saved in this browser; {error}',
  'toast.saved.cloudFail': 'Cloud sync failed',
  'toast.removed.local': 'Removed from this browser; {error}',
  'toast.removed.cloudFail': 'Cloud delete failed',
  'toast.cloudFail.status': 'Cloud sync failed ({status})',
  'toast.cloudFail.generic': 'Unable to connect to cloud flow storage',
  'toast.push.unsupported': 'This browser does not support push notifications.',
  'toast.push.unconfigured': 'Browser push is not configured',
  'toast.push.saveError': 'Failed to save notification settings',
  'toast.push.enabled': 'Browser notifications enabled. You will receive important flow alerts even when the page is closed.',
  'toast.push.enableError': 'Unable to enable browser notifications',
  'toast.repos.synced': 'Authorized repositories synced; a flow repository is no longer authorized.',
  'toast.repos.syncedCount': 'Synced {count} authorized repositories. Returned to the previous page.',
  'toast.repos.error': 'Unable to sync authorized repositories',
  'toast.repos.hint': 'After saving on GitHub, close the authorization page to return here and sync repositories.',
  'toast.queue.refreshed': 'Action queue refreshed. {count} item(s) need attention.',
  'toast.queue.syncStarted': 'Background sync started. Updated statuses will appear shortly.',
  'toast.unknownError': 'Unknown error',

  // Notification (system)
  'notif.title': 'PR Flow',
  'notif.stepUpdate': 'Step {index} status update: {detail}',
  'notif.merged': 'PR merged',
  'notif.actionsFailed': 'Actions failed',
  'notif.actionsPassed': 'All Actions passed',
  'notif.newPullReady': 'New commits are ready for a new PR',
  'toast.queue.failed': 'Failed to refresh the to-do queue',
  'toast.queue.timeout': 'Refresh timed out. Please try again shortly.',
  'toast.queue.unavailable': 'Connect GitHub before refreshing the to-do queue.',

  // Next action titles
  'nextAction.notStarted': 'Not started yet',
  'nextAction.gateFailed': 'A gate failure needs attention',
  'nextAction.canCheck': 'Next step PR is ready to review',
  'nextAction.synced': 'Flow status is in sync',

  // Toast actions
  'toastAction.copy': 'Copy',
  'toastAction.copyLabel': 'Copy feedback',
  'toastAction.close': 'Close',
  'toastAction.closeLabel': 'Close feedback',
  'toastAction.copied': 'Copied',
  'toastAction.copyFailed': 'Copy failed',

  // Native-only tooltip
  'nativeOnly.tooltip': 'Must be done on GitHub',

  // Sync health
  'syncHealth.never': 'No reconciliation has run yet. Data may be outdated.',
  'syncHealth.running': 'Reconciling workflow status…',
  'syncHealth.failed': 'Last reconciliation failed. Some status may be outdated.',
  'syncHealth.degraded': 'Reconciliation partially completed: {reconciled} stages succeeded, {failed} failed.',
  'syncHealth.interrupted': 'The last reconciliation was interrupted before it finished, so some states may be stale.',
  'syncHealth.triggerBroken': 'These triggers did not reconcile successfully last time: {triggers}.',
  'syncHealth.trigger.cron': 'Scheduled sweep',
  'syncHealth.trigger.webhook': 'GitHub events',
  'syncHealth.trigger.inbox_refresh': 'Manual refresh',
  'syncHealth.trigger.manual': 'Manual sweep',
  'syncHealth.lastSync': 'Synced {stages} stages · {time}',
  'syncHealth.secondsAgo': '{seconds}s ago',
  'syncHealth.minutesAgo': '{minutes}m ago',
  'syncHealth.hoursAgo': '{hours}h ago',
  'syncHealth.duration': 'took {ms}ms',
  'syncHealth.staleCount': '{count} stages may be stale',
  'syncHealth.stale.minutes': '~{minutes}m old',
  'syncHealth.stale.hours': '~{hours}h old',
  'syncHealth.stale.tooltip': 'This stage data may be outdated because it has not been updated for a long time.',

  // Workflow run history
  'runHistory.state.active': 'In progress',
  'runHistory.state.completed': 'Completed',
  'runHistory.state.failed': 'Failed',

  // Release timeline
  'timeline.eyebrow': 'RELEASE TIMELINE',
  'timeline.step.eyebrow': 'STEP TIMELINE',

  // Failure center
  'failureCenter.eyebrow': 'NEEDS ATTENTION',
  'failureCenter.count': '{count} items',

  // Account deletion
  'accountDelete.eyebrow': 'DANGER ZONE',
  'accountDelete.title': 'Delete your account?',
  'accountDelete.desc': 'This will permanently delete your account and all associated data, including workflows, monitoring state, events, deployment history, and encrypted sync data.',
  'accountDelete.warning': 'GitHub PRs, branches, and deployments will not be deleted. This cannot be undone.',
  'accountDelete.confirmLabel': 'Type DELETE to confirm',
  'accountDelete.cancel': 'Cancel',
  'accountDelete.confirm': 'Delete my account',
  'accountDelete.deleting': 'Deleting…',
  'accountDelete.failed': 'Account deletion failed',

  // GitHub Permissions
  'permissions.eyebrow': 'GITHUB APP PERMISSIONS',
  'permissions.title': 'Why we need these permissions',
  'permissions.actions.label': 'Actions (Read & Write)',
  'permissions.actions.desc': 'Read workflow run status, rerun failed Actions, and trigger rollback workflows.',
  'permissions.contents.label': 'Contents (Read-only)',
  'permissions.contents.desc': 'Read branch information and compare changes between branches.',
  'permissions.pullRequests.label': 'Pull Requests (Read & Write)',
  'permissions.pullRequests.desc': 'Create, read, and merge pull requests on your behalf.',
  'permissions.manage': 'Manage permissions on GitHub ↗',
  'permissions.revoke': 'You can revoke these permissions anytime through GitHub Settings → Applications.',

  // Connect page
  'connect.permissionsLink': 'What permissions do we need?',
  'connect.privacyLink': 'Privacy Policy',

  // Preflight
  'preflight.eyebrow': 'PREFLIGHT CHECKS',
  'preflight.run': 'Run preflight',
  'preflight.loading': 'Checking…',
  'preflight.pass': 'All clear',
  'preflight.fail': '{errors} errors',
  'preflight.error': 'Preflight request failed',
};

export default en;
