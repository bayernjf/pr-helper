...
function projectLane(flow: Workflow) {
  ...
  const orderInput = `<label class="lane-order-input"><span>${t('overview.board.order')}</span><input type="number" min="1" max="${workflows.length}" step="1" inputmode="numeric" value="${orderIndex + 1}" data-lane-position="${escape(flow.id)}" aria-label="${escape(t('overview.board.orderFor', { name: flow.name }))}" ${sortingDisabled ? 'disabled' : ''} /></label>`;
