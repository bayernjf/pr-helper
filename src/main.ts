...
function renderStepForm(repository: string) {
  ...
  const showSourceBranches = () => { try { sourceInput.showPicker?.(); } catch { /* Datalist suggestions remain available when the browser has no picker API. */ } };
  sourceInput.addEventListener('focus', showSourceBranches); sourceInput.addEventListener('click', showSourceBranches); sourceInput.addEventListener('input', sync); document.querySelector('#target')!.addEventListener('change', sync); sync();
...
function branchOptions() { return branches.map(branch => `<option value="${escape(branch)}"></option>`).join(''); }
