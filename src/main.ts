...
function projectLane(flow: Workflow) {
  ...
  const orderInput = laneSortMode === 'custom'
    ? `<label class="lane-order-input"><input type="number" min="1" max="${workflows.length}" step="1" inputmode="numeric" value="${orderIndex + 1}" data-lane-position="${escape(flow.id)}" aria-label="${escape(t('overview.board.orderFor', { name: flow.name }))}" ${sortingDisabled ? 'disabled' : ''} /></label>`
    : '';
