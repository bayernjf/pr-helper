...
`supabase/migrations/` is the only source of truth for PR Helper's database schema. The production API never creates or alters tables at request time.
...
| `027` | Concurrency suppression and rotation fairness via `last_reconcile_attempt_at` |
| `028` | Reconciliation leases and the `reconcile_pending_since` catch-up marker |
...
- Set `CRON_SECRET` in Vercel and the same value as the GitHub Actions secret `PR_HELPER_CRON_SECRET`. Since `030` the primary clock is `pg_cron`, which reads the same secret from the Supabase Vault entry `pr_helper_cron_secret`; the GitHub Actions schedule remains only as a fallback.
