...
生效判据：部署后在界面点一次手动刷新，看最新那条 `inbox_refresh` 的 `duration_ms`——明显超过 10 秒（约 26 秒）即新预算已在用；仍卡在 9.9 秒说明变量没生效或部署没带上。
