...
8. 等门禁的动作退避（`1e02a8a0` + `6a0ede17`，待部署）：动作 205 在 65 分钟里重试 41 次，原因是 `PR 还需要 1 个 Approval` 被标为 `retryable`、动作留在 `queued`，而 `automationRetryIsExhausted` 只看走到过判决的行。部署后核对两件事：205 的 `attempts` 增速降到每小时约 2 次；给它 Approval 后，合并仍在一两分钟内发生（走事件就地执行，不受退避影响）。
