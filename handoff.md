...
| 事项 | 一句话 | 去处 |
| --- | --- | --- |
| 推送本地两个提交 | `e2ebea7b`、`0ebbef6a` 仍未推（均为文档）。推前按项目规则先 `git pull`。 | `git log --oneline origin/main..HEAD` |
| 沙箱 6 个步骤的提交数阈值仍是 2 | 阈值实测时设的，`69q14` 步骤 0/1/2、`5xdwc` 步骤 0、`d4h70` 步骤 0、`pvmxx` 步骤 0。建议至少把 `69q14` 三个和 `5xdwc` 一个改回 1，否则沙箱后续测试会莫名不触发。只读通道改不了，须在浏览器里改。 | 《2026-08-22 自动创建 PR 的提交数阈值首次实测》 |
| 自动化动作 509 卡在 `queued` | `feature/approval-e2e → dev` 的 PR 差 1 个 Approval，与代码无关。要么 approve 让它跑完，要么当作已知残留。 | 同上 |
...
13. **分段耗时遥测与部署行写入优化（已部署，已实测，观察中）**。第 11 项的判读规则里「占比没动 → 卡的是某个会挂的 stage」这一支现在有了确切答案，而且答案不在 GitHub 那一侧。迁移 `032` 给 `reconciliation_runs` 加了 `phase_ms jsonb`，`createPhaseRecorder` 按阶段累计耗时（`prune` / `routes` / `pull` / `dbRead` / `compare` / `checks` / `deploy` / `write` / `queue` / `notify`，`deploy` 再拆 `deployRead` / `deployRuns` / `deployLookup` / `deployJobs` / `deployHealth` / `deployWrite`）。**读这些数字前必须知道两件事**，否则会得出反的结论：（a）值是并发步骤的累加，不是墙钟，和 `github_ms` 同一约定，所以总和大于 `duration_ms` 是正常的；（b）一个阶段只在它返回时才计入，因此被 deadline 抛下的步骤原本一分钱都不落账——`phase_ms.pending` 是补这个洞的，它按「未结算步骤当前所在阶段」计数（`StagePhaseTracker` / `pendingPhaseTotals`），嵌套阶段退出时恢复外层名字而不是清空，否则子阶段一返回，还在 `deploy` 里的步骤会被记成 `unstarted`。

    **实测结论（run 7571，webhook / degraded / 20 步校准 16 步 / 28084ms / `github_calls` 31452ms）**：`deployWrite` 118670、`queue` 99123、`write` 63753、`dbRead` 21841、`deployRead` 17345，而 `deployRuns` 1525、`deployLookup` 2126，`deployJobs` 与 `deployHealth` 一次都没出现（那个 10 秒健康探针确定不在路径上）。**代价压倒性地在数据库写入和内联自动化执行，不在 GitHub 往返**。连接池是 `max: 4`（`api/_lib/workflows-store.ts`），所以「写」的时间里很大一部分是在排队等连接，这也解释了为什么累加值能远超墙钟。**由此 ETag / `If-None-Match` 这条路可以彻底放下**（见下方待办清单里那条已被推翻的判断）。

    **已落地的第一刀**：部署行此前是「删掉整步的行再全部重插」，而绝大多数清扫算出来的行和库里那行一模一样。现改为先比对（`deploymentRowChanged`，按文本比，因为驱动把 numeric 返回成字符串），内容未变就不发写语句；删除也从「删光」改成只删工作流已不再声明的 provider（`staleDeploymentProviders`）——这个定向删除才是让「跳过重插」不丢行的前提。**刻意没动 `workflow_stage_states` 的 upsert**：`updated_at` 是陈旧（>15 分钟）与未收敛（>45 分钟）判定的依据，冻结它会让空闲步骤全部误报陈旧。`workflow_stage_deployment_runs` 的插入同样保持无条件，保留策略按 `updated_at` 清理，冻结会把在跑的历史提前老化掉。

    **部署后核对（2026-08-19 20:40:52 UTC 上线，sha `b739092d`）**：同为 19 步的 cron 全量清扫，`deployWrite` 由 59019（run 7640）/ 64006（run 7647）降到 **34435**（run 7724，新代码），降 45%；顺带 `write` 66552 → 48273、`duration_ms` 21579 → 17060——少发的语句同时腾出别人的排队位，所以相邻阶段一起变快。webhook 的 degraded 占比在上线前后为 7/41 → 0/11，方向一致但样本太小，不作结论。**还缺的一项**：上线后尚未等到 18 步以上的大清扫复测（现在每轮 cron 只有 9 个步骤到期），需自然出现。**下一步的两个候选先不动**：把每个 provider 的 INSERT 合成一条多行语句；调大连接池 `max`。后者不宜作为第一刀——每个 serverless 实例各持一个池，放大的是整个应用打满 Supabase pooler 的风险。

- **自动化进度条 UI 的完整方案**：只读诊断切片已于 2026-08-15 落地（`/api/inbox` 多带 `automation` 字段，受阻动作显示在失败中心、看板计数、泳道徽标、步骤抽屉和流程详情页五处），只读整体进度条已于 2026-08-19 上线生产。仍后置的是 [`docs/automated-workflow-plan.md`](docs/automated-workflow-plan.md)《自动合并进度条》里的步骤级放置、节点状态与方案表格的完全对齐、上一步/下一步解锁条件呈现，以及全部写入路径（接管对话框、重新生成、`unpause`、安全重试）——它们依赖第 2 项的「瞬时 vs 终态」分类，且生产至今没有任何一次 AI 生成失败可供验证。
...
- **让出（`degraded`）的瓶颈是时延而非调用数**：249 次 `degraded` 里有只花 10–11 次调用就超时的样本（08-18 12:01 webhook、12:13 inbox_refresh），说明单次 GitHub 往返的耗时才是约束，按调用数设上限治不到它。**2026-08-19 分段耗时落库后，这条里「先做 ETag / `If-None-Match`」的处方已被数据推翻**：时间的大头是数据库写入与内联自动化执行，GitHub 读取只占很小一段，ETag 治不到它。见第 13 项。
