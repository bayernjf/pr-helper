...
6. **drain 稳定观察若干天后，删掉 sweep 内联的执行路径**，让自动化动作只剩一条执行入口。方案已写：[`docs/superpowers/plans/2026-08-17-drop-inline-automation-execution.md`](docs/superpowers/plans/2026-08-17-drop-inline-automation-execution.md)（**待批准，未实施**）。两点纠正了原来的表述：（a）执行器只有一份，分叉的是**准入策略**——内联绕过退避 / `hasNewer` 取消 / 12 小时过期 / attempts 上限；（b）drain 的门禁退避规则是按「内联存在」写的（`workflows-store.ts:300-302`、`:353-358`），删除必须同批把 sweep 改成「信号源」，否则门禁清除后的合并延迟会从秒级掉到最长 30 分钟。文档给了 A（全删）/ B（只删内联合并）/ C（不做）三个选项，**已定 B**：47 / 26 那两个重试峰值全部来自 merge，删内联合并即可清；而创建侧 `max(attempts)` 只有 3，drain 那套规则套上去实测拦不到东西，不值得用 72% 动作从 8.6 秒变 65 秒去换。A 与 B 的难点相同（信号源改造两者都要做），A 没有多省工。
