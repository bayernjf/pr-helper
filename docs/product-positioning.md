...
> 状态注记（2026-08-12）：团队/角色/共享流程能力已实现并部署，真实多账号验收仍待完成；服务端 AI 凭据和后台自动创建 PR 已在本地落地，待部署验收。功能上线状态和 backlog 以 [当前状态](current-state.md) 为准。

> 更新（2026-08-15）：后台自动创建 PR 与逐步骤自动合并均已部署并在生产跑通（`create-pr` 47 次、`merge-pr` 37 次成功），迁移基线升至 `031`，校准时钟改由 `pg_cron` 驱动。上面这条注记里的「待部署验收」已作废。
...
1. **后台自动创建 PR 与自动合并真实验收**：`024`–`031` 已执行并部署，生产已跑通规则快照、后台触发、失败暂停和门禁为红时不合并；沙箱验完必需审批数同时读 ruleset。剩幂等命中记成功一项需另造场景。 🟡 基本完成
...
6. **稳定阶段身份正式切换**：`019_stage_identity_primary_keys.sql` 已执行，服务端从 `stage_index` 切换到 `stage_id`，Production 主链路已回归。 ✅
7. **统一服务端状态模型**：由服务端输出阶段决策，前端只负责展示和操作，减少跨页面状态不一致。 🟡 待加固
8. **浏览器 E2E 与安全保护**：覆盖高风险 UI 链路，并补充并发控制、幂等、CSRF 和限流。 🟡 待加固
9. **审计、保留和团队权限**：操作审计、历史数据清理及团队/角色/流程级权限均已实现并部署；待真实外部条件验收。 🟡 验收中
