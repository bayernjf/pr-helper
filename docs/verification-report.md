...
2026-08-12 已确认 PR 流程自动化方案，但代码尚未实现，因此不属于本报告的通过项。第一步先验证服务端加密 AI 凭据 PoC，包括保存、测试、替换、删除、日志脱敏和安全暂停；其后使用现有步骤编辑器实现自动创建 PR、逐步骤自动合并、合并后自动推进、新提交阈值、策略快照和动作幂等，不引入画布。自动创建 PR 必须验证服务端自动流程凭据、AI 自动生成标题/描述、自动确认创建和有效生成规则四项前置条件，缺少任一项时不得启用；条件满足也不会自动勾选，必须用户主动开启。详见 [`automated-workflow-plan.md`](automated-workflow-plan.md)。
