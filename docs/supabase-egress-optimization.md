...
> 创建日期：2026-08-21
> 最后更新：2026-08-21（新增《实测：扫掠频率与 webhook 事件分布》《需求澄清：事件驱动，不是无人值守》《第二轮：按真实需求降配（A1 / A2 / A3）》《第三轮：把过滤与批次下推到 SQL》《数据建模债》《化债方案》《第六轮：兜底扫掠瘦身》；补记 A1 的生产实测、它引发的收敛阈值回归与处置，第三轮的索引命中实测，以及第三轮之后的投影重算）
> 适用范围：PR Helper 的 Supabase 出站流量、看板轮询、状态投影、历史数据读取和多用户扩展。
> 当前状态：三刀已落地（`1e5c6758` / `2b81be2c` / `98b5d245`）；第二轮 A1 / A2 / A3 已落地（`e65daa6c` / `b51bcaed` / `538a33eb`）；第三轮（SQL 下推）已落地并**已部署生产**（`613fd350`），迁移 034 已应用，索引命中已实测；A1 的收敛阈值回归已处置（`STAGE_UNCONVERGED_THRESHOLD_SECONDS=9000`）。第三轮之后投影约 **0.38 GB/月**（1.71 GB 是第二轮的数字，见《第三轮之后的投影重算》）。执行顺序为 **6③ → 4 → 5 → 6④**：6③ 已落地并部署；第四轮表与代码已随 035 落地，收缩迁移 **040 已写好等应用**；第五轮 036–039 已落地（`payload` 列的删除仍挂着）；6④ 的前置已就绪，但其收益前提在 6③ 之后已不成立，**建议降级或砍掉**，待定。
...
- **成本对比**：33 仓库 × 2 = 66 次调用走完一圈，取代现在「8 个流程 218 次 × 4.4 圈 ≈ 950 次」。一圈从 75 分钟变成一次 sweep。
...
**第五轮（拆成关系表，对外开放前）**：见《化债方案 · 第五轮》。数周级，按 expand → migrate → contract 四步走。**036 / 037 / 038 已于 2026-08-21 落地**，扫掠与 webhook 投影已改读列，第三轮放弃的分支条件下推随之成立；039 contract 的范围已收窄到只删一个已死索引；`payload` **列**的删除仍然挂着，是独立的一步。第六轮 ④ 的候选集所依赖的列化已就绪。
