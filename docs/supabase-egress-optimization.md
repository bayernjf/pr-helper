...
- **2026-08-22 生产实测（DevTools PerformanceObserver，详情页）**：9 分钟内 4 次 `/api/inbox`，全部 `hidden=false`，隐藏期零请求；间隔 308 s / 192 s，即每次都对应一次「切回浏览器」，证实已无时钟。单次响应体 **301.6 kB 原始 / 24.3 kB gzip**（这是服务端到浏览器的响应体，与本文其他地方的 594.3 → 274.0 kB 不是同一口径，后者是数据库到服务端）。同时暴露一个重复读：切回标签页会同时触发 `focus` 与 `visibilitychange`，两次一模一样的请求落在同一秒（`#1`/`#2` 字节完全相同），已在 `refreshStatuses` 加 in-flight 守卫修掉（`f40bf119`），用户主动触发的刷新不受影响。
- **2026-08-22 修复后复测（同一方法，生产部署 `02e2c960`，21:35:16Z 转 success）**：整个页面生命周期内启动只有 1 次 `/api/inbox`（`t=6679ms`），之后每次切回前台恰好 1 次（间隔 60.7 s / 14.3 s），**同秒成对已消失**；另有 1 次 `?refresh=1&repository=…`，来自用户主动点击，符合预期。守卫已生效。复测过程中有一次样本仍带成对请求，时间在部署转 success 之后，最合理解释是该标签页仍在跑 service worker（[`public/push-sw.js`](../public/push-sw.js)）缓存的旧 bundle，未能直接证实，记录备查。
- **未做的一项：回到前台的最小间隔**。上述复测里 14.3 s 那次是「切出去又很快切回」，请求不重复但内容几乎不可能变化，可以用一个最小间隔（30 s 或 60 s 内不再自动读）拦掉。**2026-08-22 决定不做**，理由是它拿新鲜度换出站量，而出站量还没有度量结论——先等一周 Supabase Usage（最早 2026-08-28）。若届时日增量已回到 5 GB/月以下，这一项直接作废；若仍超，再评估，届时的实现范围只限「回到前台」这一种触发，`refresh=1` 的校准读和操作后刷新一律不拦。单用户收益很小（该样本 4 条里省 1 条），但随用户量线性放大。
...
## `version` 接入乐观锁（2026-08-22 已落地、已部署、已验收）
...
**验收结果（2026-08-22，`e9ffcb76` 经 PR #336 → #337 合入 `main` 后）**：两条浏览器行为均由用户确认与改前一致——跨窗口保存仍报「流程已被其他窗口更新」，排序与增删步骤无误报冲突。库侧证据（只读通道）：`d4h70` 部署后从 version 73 连续保存到 **80**，`workflow_versions` 恰好 80 行、`MAX(version)` 也是 80——**无跳号、无主键碰撞**，即 `saveWorkflowVersion` 改写「调用方在锁内定好的号」之后仍与列同步；全表 35 行仍是 0 个 NULL、与历史 `MAX` 零处不一致。

**一条方法论教训**：用户第一次报「符合预期」时，那四个提交还没推送，生产跑的仍是旧代码——那次实际验的是**改前基线**，被 `git fetch` 核对出来才没记成错的验收。改前基线本身有用（它钉住了「应当保持不变」的那两条表现），但不能当改后证据。凡「已部署」的说法都要先核对提交是否真在远端并合入 `main`。
...
## 八月实际账单验证与最终结论（2026-08-31）

> 待办《连续观察一周 Supabase Usage 日增量（最早 2026-08-28 出结论）》在此收口。
> 数据口径：精确值来自 Supabase Usage 图表悬停读数（8/19 峰值 1 738 MB、8/20 686.51 MB、8/22 121.816 MB、8/26 99.7 MB、8/29 61.345 MB），其余天数按柱高与 477 MB / 954 MB / 1.7 GB 刻度线的目视比例估算。估算值只用于趋势判断，不是账单口径；commit 计数来自 GitHub API，webhook 计数来自 `github_webhook_deliveries`。

### 8/19–8/31 按天对比

| 日期 | commit | webhook | egress (MB) | 环比 | 每 wh 出站 (MB) | 备注 |
| --- | --- | --- | --- | --- | --- | --- |
| 08-19 | 108 | 1 071 | 1 738.0 | — | 1.623 | 精确（峰值）|
| 08-20 | 55 | 1 076 | 686.5 | −60.5% | 0.638 | 精确 |
| 08-21 | 122 | 938 | ~400 | −41.7% | 0.426 | 估算（优化部署日）|
| 08-22 | 119 | 2 545 | 121.8 | −69.6% | 0.048 | 精确 |
| 08-23 | 138 | 1 306 | ~95 | −22.0% | 0.073 | 估算 |
| 08-24 | 99 | 572 | ~78 | −17.9% | 0.136 | 估算 |
| 08-25 | 217 | 1 254 | ~102 | +30.8% | 0.081 | 估算 |
| 08-26 | 368 | 856 | 99.7 | −2.3% | 0.116 | 精确 |
| 08-27 | 62 | 1 392 | ~48 | −51.9% | 0.034 | 估算 |
| 08-28 | 40 | 90 | ~35 | −27.1% | 0.389 | 估算（webhook 极少，多为浏览器/手动活动）|
| 08-29 | 144 | 622 | 61.3 | +75.1% | 0.099 | 精确 |
| 08-30 | 77 | 481 | ~52 | −15.2% | 0.108 | 估算 |
| 08-31 | 7 | 129 | ~30 | −42.3% | 0.233 | 估算 |

### 最终优化效果结论

1. **三段跳水对应优化部署节奏**：8/19→8/20（−60.5%）、8/20→8/21（−41.7%）、8/21→8/22（−69.6%），与三刀 / 第二轮 / 第三轮的部署时间线吻合。
2. **总降幅 −94.0%**：优化前（8/19–8/20）平均 1 212 MB/天 → 优化后（8/22–8/31）平均 72 MB/天；对峰值日 8/19 为 −95.8%。
3. **降幅全部来自技术优化，不是活动量下降**：优化后 commit 日均 127（优化前 81.5）、webhook 日均 925（优化前 846）不降反升，egress 却降 94%。单位活动成本是更干净的指标：每 webhook 出站 1.62 → 0.078 MB（剩 4.8%），每 commit 出站 16.09 → 0.57 MB（剩 3.5%）。
4. **平台期特征**：8/22 起稳定在 30–120 MB/天，波动 ±30% 左右且都发生在绝对低位——8/25（+30.8%）与 8/29（+75.1%）是活跃日（commit 217 / 144）的正常浮动，不是成本回弹。
5. **月度投影**：72 MB/天 × 30 ≈ **2.1 GB/月（5 GB 额度的 42%）**。比第三轮后的理论投影 0.38 GB/月高，差异来自八月实际活动量大（webhook 925 次/天 × 0.078 MB ≈ 72 MB/天本身就是主要构成）；但仍在额度内，**宽限期 2026-09-20 之前无需升档或开按量付费**。

### 后续要做

- **[作废]「回到前台的最小间隔」**：验收标准写明"若届时日增量已回到 5 GB/月以下，这一项直接作废"——2.1 GB/月已远低于 5 GB，不再需要。
- **[重定级] `payload` 列删除**：原定"等 8/28 Usage 结论一并决定"。结论已出：egress 已收口，删列的收益是收口双表示（消除 payload 与关系表的长期双真相）而非省流量，不再受宽限期驱动，按代码债优先级重排；方案不变（[`docs/superpowers/plans/2026-08-22-drop-workflow-payload-column.md`](docs/superpowers/plans/2026-08-22-drop-workflow-payload-column.md)）。
- **9 月新账单周期精确复核**：9 月 Usage 日增量可读出精确值，替换本文档估算列；并在 2026-09-20 宽限期结束前看一次 9 月累计，确认 2.1 GB/月的投影没有偏差。
- **保持现状、无需重启**：A1 的 `*/30` 时钟、A2 事件裁剪、A3 无时钟轮询、第三轮 SQL 下推、第六轮 ③ 短路均维持；多用户方向仍按《多用户场景下真正的阻塞》的启动条件（第二个真实用户出现）再评估。
