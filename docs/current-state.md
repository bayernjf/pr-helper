...
> 最后更新：2026-08-21（服务端自动创建 PR 与逐步骤自动合并均已在生产端到端跑通；自动化队列 drain 是唯一执行入口；流程详情页的只读进度条已上线；Supabase Free Plan 出现 Egress 超额，已实测定位到「轮询隐藏不停」与「同一请求内重复读 payload」两项，见《Supabase Egress 与多用户扩展方案》）
...
- **结论：综合 7.5/10**。产品已超过 MVP，可继续作为受控生产工具使用；下一步优先降低 Supabase Egress（先轮询隐藏即停 + 60 秒，再请求内去重），再继续收敛 reconciliation 写入和完成外部条件验收，不建议立即投入画布或模板市场。
...
2026-08-21 已按各查询的实际列和 LIMIT 在生产库上量过字节，权重与上述判断不同：一次 `/api/inbox` 约 588 kB，其中 `pr_helper_workflows.payload`（35 行、70.8 kB）在**同一次请求内被 5 个 list 函数各取一遍**，占 354 kB，重复的 4 遍即 283 kB / 48%；历史数据合计只有 143 kB / 24%。按 30 秒轮询折算约 70 MB/小时，5 GB 约等于 71 小时页面打开时间，一个后台标签页挂 12 小时约 847 MB——现有超额无需多用户即可解释。同时确认浏览器轮询在页面隐藏时**不会停**，`visibilitychange` 只是回到前台额外刷一次。

因此第一阶段的落地顺序已修正为先做前两项，`/api/board` 拆分降为可选：

- **轮询隐藏即停、间隔改 60 秒、回到前台重启定时器并立刷一次**：改动最小而真实收益最大（挂机 847 MB/晚 → 0，前台 70 → 35 MB/小时）。
- **请求内去重**：handler 先取一次 payload 与 stage_states 传进各 list 函数，单次 -48%，接口形状与 UI 行为零变化。
- 上述两项合计约 70 → 9 MB/小时。之后观察一周，再决定是否需要下列改动。
