...
import { listActionableStages, listWorkflowStageStates, reconcileWorkflowStages } from './_lib/workflows-store.js';
...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  ...
  try {
    ...
    const [items, states] = await Promise.all([listActionableStages(process.env, identity), listWorkflowStageStates(process.env, identity)]);
    response.status(200).json({ items, states });
  } catch (error) {
