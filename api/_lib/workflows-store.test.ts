...
describe('stored workflow validation', () => {
  ...
  // Measured on `bayernjf/bayjf` 2026-08-22: a 404 health probe was persisted as `state = failure` on the
  // deployment row, but the value handed back to the caller was recomputed from the Actions run alone, so
  // `mergeChecksWithDeployments` saw `success` and `checks_state` stayed green. The health probe was a
  // badge, not a gate — it blocked neither the next stage's unlock nor auto-merge. Both the row and the
  // return value must come from this one reading.
  it('folds a failed health probe into the deployment state', () => {
    expect(deploymentStateWithHealth('success', 'failure')).toBe('failure');
    expect(deploymentStateWithHealth('success', 'success')).toBe('success');
    expect(deploymentStateWithHealth('success', null)).toBe('success');
    expect(deploymentStateWithHealth('failure', 'success')).toBe('failure');
    // A health probe only runs once the run succeeded, so it can never rescue a pending run.
    expect(deploymentStateWithHealth('pending', 'failure')).toBe('pending');
  });
