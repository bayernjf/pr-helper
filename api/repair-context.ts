...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  ...
  try {
    ...
    const payload = body(request) as { workflowId?: unknown; stageIndex?: unknown };
    if (typeof payload.workflowId !== 'string' || typeof payload.stageIndex !== 'number') throw new Error('无效的修复任务请求');
    const context = await codexRepairContext(process.env, { login: session.login, githubUserId: session.githubUserId, installationId: session.installationId }, payload.workflowId, payload.stageIndex);
